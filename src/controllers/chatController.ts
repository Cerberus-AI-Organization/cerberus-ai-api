...
export const getChatMessages = async (req: Request, res: Response) => {
    try {
        ...
        const resultChatUsers = await pool.query(`SELECT * FROM chat_users WHERE chat_id = $1 AND user_id = $2`,
            [Number(id), user.id]);
...
export const postChatMessage = async (req: Request, res: Response) => {
    try {
        ...
        const result = await pool.query(`SELECT * FROM chat_users WHERE chat_id = $1 AND user_id = $2`,
            [id, user.id]);
        ...
        await pool.query(
            `INSERT INTO messages (chat_id, sender_type, sender_id, content) VALUES ($1, 'user', $2, $3) RETURNING *`,
            [id, user.id, content]
        );
        ...
        for await (const chunk of runOllamaStream(Number(node_id), model, session, [
            ...
            generatedMessage += chunk.content;
            res.write(JSON.stringify(chunk) + "\n");
        }

        await pool.query(
            `INSERT INTO messages (chat_id, sender_type, sender_id, content) VALUES ($1, 'ai', $2, $3) RETURNING *`,
            [id, null, generatedMessage]
        );
